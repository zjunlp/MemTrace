...
from smartcomment import comment_variable
...
def get_update_memory_messages(retrieved_old_memory_dict, response_content, custom_update_memory_prompt=None):
    ...
    comment_variable(
        ...
        variable_name="custom_update_memory_prompt", 
        id_strategy=lambda x: "memory-update-decision-prompt",
        category="prompt",
        comment=(
            "A prompt template for the memory system to decide whether to add, " 
            "update, delete, or no change the memory based on the new extracted facts "
            "and the existing memory similar to the new extracted facts."
        ),
        metadata={
            "op_type": "update",
        },
    ...
    comment_variable(
        f"Find {len(retrieved_old_memory_dict)} existing memories similar to "
        "the new extracted facts.", 
        variable_name="existing_memories",
        category="similar_memories", 
        class_name="existing_memories",
        comment=(
            "For each fact, the memory system retrieves top-k similar memories "
            "from the existing memory store. Then, these memory units are merged into "
            "this variable. This variable just presents the total number of similar "
            "memories found. For detailed information, please refer to the corresponding "
            "memory update instruction with these existing memories injected."
        ), 
    )
